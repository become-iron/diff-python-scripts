...
def create_message(string: str, coding='cp1251') -> tuple:
    ...
    Примеры:
        ...
        >>> print(*data[1])
        11001010 11101110 11111000 11101010 11101000 11101101 00100000 11000010 00101110 11001000 00101110
        >>> print(data[2])
        11 байт (88 бит)
        """
    codes = tuple(ord(sym.encode(coding)) for sym in string)
    hex_ = tuple(str(hex(code)).lstrip('0x').upper() for code in codes)
    bin_ = tuple(str(bin(code)).lstrip('0b').zfill(8) for code in codes)
    result = (
        hex_,
        bin_,
